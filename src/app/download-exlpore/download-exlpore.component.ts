...
export class DownloadExlporeComponent implements OnInit {
  ...
  startBuildingReport() {
    ...
    this.http
      .post(
        `${devApiUrl}/api/filter-data-to-csv`,
        {
      ...
      .subscribe({
        next: (response: any) => {
          ...
          this.filterService.setCurrentStep(4);
  ...
  sendPurchaseReportEmail(downloadLink: string) {
    ...
    this.http
      .post(
        `${devApiUrl}/api/send-purchase-report-email`,
        {
          to: this.email,
          name: this.userName,
  ...
  //     window.location.href = `${devApiUrl}/api/download-report/${this.fileName}`;
  //     this.filterService.showInfoToast("Downloaing your report !!");
  //   }
  // }
  ...
  downloadReport() {
    if (this.fileName) {
      ...
      this.filterService.showInfoToast('Downloaing your report !!');
      const downloadUrl = `${devApiUrl}/api/download-report/${this.fileName}`;

      this.http.get(downloadUrl, { responseType: 'blob' }).subscribe({
        next: (blob) => {
          saveAs(blob, this.fileName || 'downloaded_report'); // Use file-saver for better handling
          this.isFileDownloading = false;
          this.filterService.showInfoToast('Download complete!');
        },
        error: (err) => {
