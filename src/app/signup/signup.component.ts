...
export class SignupComponent {
  ...
  handleSignUp() {
    ...
    if (!this.otpSent) {
      this.http
        .post(`${devApiUrl}/api/auth/signup`, { 
          email: this.email,
          password:this.password ,
          userName:this.username ,
          companyName :this.companyName
        ...
        .subscribe({
          next: (response: any) => {
           ...
           this.router.navigate(['verify-signup-otp'], { queryParams: { email: this.email } })
          ...
          error: (error) => {
            ...
            this.successMessage = '';
            this.filterService.showErrorToast("Signup failed , try again !!");
            this.isSignUpFormSubmit = false;
          },
        });
    } else {
