...
export class GenerateReportComponent implements OnInit {
  ...
  filteredIndicationList(): string[] {
    ...
    return this.IndicationList.filter((item: any) =>
      item?.toLowerCase().startsWith(this.searchIndication.toLowerCase())
    );
  }
  ...
  filteredMOAList(): string[] {
    ...
    return this.MOAList.filter((item: any) =>
      item.toLowerCase().startsWith(this.searchMOA.toLowerCase())
    );
  }
  ...
  async getSelectedTopicFilters() {
    ...
    const apiUrl: string = `${devApiUrl}/api/get-pricing-distinct-view`;
  ...
  sendEmailofTalk(plan: any) {
    ...
    this.http.post(`${devApiUrl}/api/email-of-talk`, userData ,{headers}).subscribe({
      next: (response: any) => {
        console.log('resposne is ---- \n ', response);
  ...
  getTableData(indication: string, indication_value: string) {
    ...
    this.http.post(`${devApiUrl}/api/filter-data`, filtersData, {headers}).subscribe({
      next: (response: any) => {
        console.log('resposne is ---- \n ', response.data);
