...
export class PurchaseHistoryComponent implements OnInit {
  ...
  fetchReportPurchaseHistory(email: string) {
    ...
    this.http
      .post(`${devApiUrl}/api/get-report-purchase-history`, {
        email: email,
      },{headers})
      .subscribe({
        next: (response: any) => {
  ...
  downloadReport(fileName: string) {
    if (fileName && fileName.length !== 0 && fileName !== null) {
      ...
      window.location.href = `${devApiUrl}/api/download-report/${fileName}`;
    }
  }
  ...
  viewReport(fileName: string) {
    ...
    if (fileName && fileName.length !== 0 && fileName !== null) {
      this.http.get(`${devApiUrl}/api/view-report/${fileName}`,{headers}).subscribe({
        next: (response: any) => {
  ...
  getVisiblePages(): number[] {
    const pagesToShow = 4; // Number of page numbers to display
